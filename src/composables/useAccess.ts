...
export function useAccess() {
  ...
  const flattenedNodes = computed(() =>
    flattenTree(menuTree.value).filter((node) => node.permissions.canView && Boolean(node.path)),
  );

  const firstAccessiblePath = computed(() => flattenedNodes.value[0]?.path ?? '/dashboard');
  ...
  const hasPathAccess = (path: string) => {
    ...
    return flattenedNodes.value.some((node) => {
      ...
      return nodePath && (normalizedPath === nodePath || normalizedPath.startsWith(`${nodePath}/`));
    });
  };
