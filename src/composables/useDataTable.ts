...
export interface UseDataTableOptions<T, Query> {
  fetcher: (query: Query) => Promise<ApiResponse<T[]>>;
  initialPage?: number;
  initialLimit?: number;
...
export interface UseDataTableReturn<T> {
  ...
  fetchData: (override?: Partial<DataTableState>) => Promise<ApiResponse<T[]>>;
  setPage: (value: number, opts?: { runFetch?: boolean }) => Promise<ApiResponse<T[]> | undefined>;
  setLimit: (value: number, opts?: { runFetch?: boolean }) => Promise<ApiResponse<T[]> | undefined>;
  setKeyword: (value: string, opts?: { runFetch?: boolean }) => Promise<ApiResponse<T[]> | undefined>;
  setSort: (
  ...
  resetFilters: (opts?: { runFetch?: boolean }) => Promise<ApiResponse<T[]> | undefined>;
...
export function useDataTable<T, Query>({
  fetcher,
  initialPage = 1,
  initialLimit = 20,
  initialKeyword = '',
  ...
  const updateMeta = (meta?: ApiResponse<T>['meta']) => {
    if (meta?.page) {
    ...
    totalPages.value =
      meta?.totalPages ?? (limit.value > 0 ? Math.max(1, Math.ceil(total.value / limit.value)) : 1);
  };

  const fetchData = async (override?: Partial<DataTableState>): Promise<ApiResponse<T[]>> => {
    loading.value = true;
    error.value = null;
  ...
  const setSort = (
    ...
    if (payload.sortOrder !== undefined) sortOrder.value = payload.sortOrder;
    page.value = 1;
    return queueFetch(opts?.runFetch);
  };

  const setFilters = (payload: Record<string, unknown>, opts?: { runFetch?: boolean }) => {
