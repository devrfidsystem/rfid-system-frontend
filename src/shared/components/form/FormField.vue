<template>
  <div :class="containerClass">
    ...
    <div :class="contentColumnClass">
      <slot :id="id" :describedById="describedById" :hasError="hasError" />

      <p v-if="hint" :id="hintId" class="text-xs text-gray-500 mt-1">{{ hint }}</p>
      <p v-if="displayError" :id="errorId" class="text-xs text-error-600 mt-1" aria-live="polite">
        {{ displayError }}
      </p>
    </div>
  </div>
...
const errorId = computed(() => (displayError.value ? `${id.value}-error` : undefined));
...
const containerClass = computed(() => {
  ...
  if (props.mode === 'inline') {
    classes.push('md:grid', 'md:grid-cols-[220px_1fr]', 'md:items-start', 'md:gap-3');
  }
  return classes.join(' ');
});
