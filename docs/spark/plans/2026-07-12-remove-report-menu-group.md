...
### Task 1: Remove `opname-variance` from `reportConfig.ts`
...
- Produces: `ReportKey` type narrowed to 8 values (no `"opname-variance"`); `reportConfigs` narrowed to the same 8 keys. Task 2 relies on this narrowed `ReportKey` when deleting the routes that iterate `Object.keys(reportConfigs)`.
...
Run: `npx eslint --fix src/views/report/reportConfig.ts`
Expected: no output.

- [ ] **Step 5: Commit**

```bash
git add src/views/report/reportConfig.ts
