...
- Keep the app's own brand blue (`primary` variant) on the submit button — do not recolor to ClickUp's palette. Only adopt the pill (`rounded-full`) shape.
    ...
    <div class="auth-page">
        <div class="auth-page__container">
            ...
            <p class="auth-page__footer">
                © {{ new Date().getFullYear() }} ALIR Smart System · All
                rights reserved
            </p>
        </div>
    </div>
...
Changes from the previous version: `form-subtitle` prop removed from `AuthShell` in favor of the `#subtitle` slot (with the register link now living there instead of in a row next to "Ingat saya"); both `Input`s get `label-class="sr-only"`; the password `Input` uses a dynamic `:type` bound to `showPassword` and a `#trailingIcon` eye-toggle button; the old `<div class="flex items-center justify-between text-sm">` row (remember-me + register link side by side) is replaced by the "Ingat saya" checkbox alone as its own row; the submit `Button` gains `rounded-full`.
