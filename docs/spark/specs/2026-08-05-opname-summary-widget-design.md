...
2. **Status Breakdown** — badge per status + count + percentage. Status-to-tone mapping is defined locally in this component rather than imported from the sibling `TransactionSummaryWidget` (a same-output mapping isn't worth coupling two unrelated view folders together — note the sibling's sets happen to already be a superset covering Opname's statuses too, so this is a duplication of convenience, not a difference in actual output): `closed`/`reconciled` → success, `counting`/`draft` → warning, `canceled` → error, unmapped → neutral.
